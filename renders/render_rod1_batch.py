...
CFG={
    '2sec': dict(frame=os.path.join(ROOT,'tesserae_2sec_frameonly_rod1.stl'), num_sections=2, prefix='tesserae_2sec_rod1_black'),
    '6sec': dict(frame=os.path.join(ROOT,'tesserae_6sec_frameonly_rod1.stl'), num_sections=6, prefix='tesserae_6sec_rod1_black'),
}[variant]
...
def add_ground():
    ...
    g.data.materials.append(mat('Ground',(0.19,0.19,0.19),0,0.65))
...
def render(name, cam_params):
    ...
    frame=import_stl(CFG['frame'], mat('Frame',(0.08,0.08,0.09),0.9,0.42))
    spots=[]
    for i in range(CFG['num_sections']):
        z_center=i*section_height + section_height/2
        s=import_stl(SPOT, mat(f'Spot{i}',(0.12,0.12,0.13),0.75,0.48))
